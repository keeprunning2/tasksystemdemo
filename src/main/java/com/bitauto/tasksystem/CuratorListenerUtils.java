...
public class CuratorListenerUtils {
    ...
    private static CuratorFramework clientTwo() {
        ...
        String scheme = "tech";
        byte[] auth = "lipp:li.1234".getBytes();
        int connectionTimeoutMs = 5000;
        String connectString = "localhost:2181";
        String namespace = "com.bitauto.tasksyste";
    ...
    private static void setListenterThreeTwo(CuratorFramework client) throws Exception {
        ExecutorService pool = Executors.newCachedThreadPool();
        //设置节点的cache
        final NodeCache nodeCache = new NodeCache(client, "/zkfile/lock", false);
    ...
    private static void setListenterThreeThree(CuratorFramework client) throws Exception {
        ExecutorService pool = Executors.newCachedThreadPool();
        //设置节点的cache
        TreeCache treeCache = new TreeCache(client, "/zkfile/lock");
        ChildData childData= treeCache.getCurrentData("D:\\Code\\JavaCode\\jar\\zookeeper-3.4.10\\zkfile\\lock");
        if(childData!=null) {
            System.out.println(childData.getPath());
            System.out.println(childData.getData().length);
        }
        //设置监听器和处理过程
        treeCache.getListenable().addListener(new TreeCacheListener() {

            public void childEvent(CuratorFramework client, TreeCacheEvent event) throws Exception {
                System.out.println("come in event");
                ChildData data = event.getData();
                if (data != null) {
                    switch (event.getType()) {
                        case NODE_ADDED:
